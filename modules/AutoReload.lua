if string.lower(RequiredScript) == "lib/units/beings/player/states/playerstandard" then
	...
	function PlayerStandard:_check_action_primary_attack(t, input, ...)
		if WolfgangHUD:getSetting({"GAMEPLAY", "AUTO_RELOAD"}, true) then
			...
			if action_wanted then
				...
				if not action_forbidden then
					if self._equipped_unit then
						...
						if weap_base.clip_empty and weap_base:clip_empty() and not weap_base:out_of_ammo() and not self:_is_using_bipod() and fire_mode ~= "single" then
